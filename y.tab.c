...
{
       0,    39,    39,    40,    41,    42,    45,    59,    78,    79,
      82,    85,    86,    87,    90,    93,    94,    95,    98,    99,
     100,   101,   102,   103,   111,   112,   119,   120,   121,   124,
     127,   128,   131,   131,   131,   134,   134,   134,   137,   138,
     139,   140,   141,   142,   143,   144,   145,   148,   149,   150,
     151,   152,   153,   156,   172,   177,   178,   179,   184,   202,
     226,   229,   230,   233,   245,   246,   249,   264,   265,   266,
     267,   268,   269,   270,   271,   272,   275,   276,   279,   304,
     317,   340,   371,   374,   377
};
#endif
...
static const char *const yytname[] =
{
  ...
  "FUNCION", "DO", "ARRAY", "ELSE", "STRUCT", "STRUCT_VAR", "FOR", "GE",
  "EQ", "NE", "INC", "DEC", "'='", "'('", "')'", "','", "'{'", "'}'",
  "';'", "'>'", "'<'", "'+'", "'-'", "'*'", "'/'", "'['", "']'", "$accept",
  "start", "Funcion", "lista_parametro", "parametro", "Tipo",
  "CompoundStmt", "StmtList", "stmt", "dowhile", "if", "for", "$@1", "$@2",
  "while", "$@3", "$@4", "expr1", "asignacion", "asignacion1", "paralist",
  "param", "asignacion2", "exp", "tipoconst", "Declaracion", YY_NULLPTR
};
#endif
...
#line 45 "parser.y" /* yacc.c:1646  */
    {
	if ((yyvsp[-4].ival)!=retornarfunc_tipo(ct))
	{
	...
	{
		insertar((yyvsp[-3].str),FUNCION);
		insertar((yyvsp[-3].str),(yyvsp[-4].ival));
	}
	}
...
#line 59 "parser.y" /* yacc.c:1646  */
    {
	if ((yyvsp[-5].ival)!=retornarfunc_tipo(ct))
	{
	...
	{
		insertar((yyvsp[-4].str),FUNCION);
		insertar((yyvsp[-4].str),(yyvsp[-5].ival));
                for(j=0;j<=k;j++)
                {insertarp((yyvsp[-4].str),plist[j]);}
                k=-1;
	}
	}
...
#line 82 "parser.y" /* yacc.c:1646  */
    {plist[++k]=(yyvsp[-1].ival);insertar((yyvsp[0].str),(yyvsp[-1].ival));insertaralcance((yyvsp[0].str),i);}
...
#line 112 "parser.y" /* yacc.c:1646  */
    {
                          int sct=retornaralcance((yyvsp[-1].str),stack[top-1]);	//stack[top-1] - current alcance
		          int tipo=retornartipo((yyvsp[-1].str),sct);
...
#line 157 "parser.y" /* yacc.c:1646  */
    {
		int sct=retornaralcance((yyvsp[-2].str),stack[top-1]);
		int tipo=retornartipo((yyvsp[-2].str),sct);
		if((!(strspn((yyvsp[0].str),"0123456789")==strlen((yyvsp[0].str)))) && tipo==258)
			{printf("\nError : Tipos no coinciden : Linea %d\n",printline()); errc++;}
                else if (tipo==273)  {printf("\nError : Tipos no coinciden : Linea %d\n",printline());errc++;}
		if(!buscar((yyvsp[-2].str)))
		{
			int currscope=stack[top-1];
			int alcance=retornaralcance((yyvsp[-2].str),currscope);
			if((alcance<=currscope && end[alcance]==0) && !(alcance==0))
				verificar_alcance_actua((yyvsp[-2].str),(yyvsp[0].str),currscope);
		}
	}
...
#line 185 "parser.y" /* yacc.c:1646  */
    {
                int sct=retornaralcance((yyvsp[-5].str),stack[top-1]);
		int tipo=retornartipo((yyvsp[-5].str),sct);
                //printf("%s",$3);
                int rtype;
                rtype=retornartipof((yyvsp[-3].str)); int ch=0;
                //printf("%d",rtype);
		if(rtype!=tipo)
			{ printf("\nError : Error Tipos no coinciden : Linea %d\n",printline()); errc++;}
		if(!buscar((yyvsp[-5].str)))
		{
		  for(j=0;j<=l;j++)
                  {ch = ch+verificarp((yyvsp[-3].str),flist[j],j);}
...
#line 203 "parser.y" /* yacc.c:1646  */
    {
                int sct=retornaralcance((yyvsp[-3].str),stack[top-1]);
		int tipo=retornartipo((yyvsp[-3].str),sct); int ch=0;
		if(!buscar((yyvsp[-3].str)))
		{
		  for(j=0;j<=l;j++)
                  {ch = ch+verificarp((yyvsp[-3].str),flist[j],j);}
...
#line 234 "parser.y" /* yacc.c:1646  */
    {
                ...
                {
                	int sct=retornaralcance((yyvsp[0].str),stack[top-1]);
                	flist[++l]=retornartipo((yyvsp[0].str),sct);
                }
	}
...
#line 250 "parser.y" /* yacc.c:1646  */
    {
		if(c==0)							//check compatibility of mathematical operations
		{
			c=1;
			int sct=retornaralcance((yyvsp[0].str),stack[top-1]);
			b=retornartipo((yyvsp[0].str),sct);
		}
		else
		{
			int sct1=retornaralcance((yyvsp[0].str),stack[top-1]);
			if(b!=retornartipo((yyvsp[0].str),sct1)){}
				{printf("\nError : Tipos no Coinciden : Linea %d\n",printline());errc++;}
		}
	}
...
#line 280 "parser.y" /* yacc.c:1646  */
    {
		...
		{
			int currscope=stack[top-1];
			int previous_scope=retornaralcance((yyvsp[-3].str),currscope);
			if(currscope==previous_scope)
				{printf("\nError : Redeclaracion of %s : Line %d\n",(yyvsp[-3].str),printline());errc++;}
			else
			{
				insertar_dup((yyvsp[-3].str),(yyvsp[-4].ival),currscope);
				verificar_alcance_actua((yyvsp[-3].str),(yyvsp[-1].str),stack[top-1]);
			}
		}
		else
		{
			...
			insertar((yyvsp[-3].str),(yyvsp[-4].ival));
			insertaralcance((yyvsp[-3].str),alcance);
			verificar_alcance_actua((yyvsp[-3].str),(yyvsp[-1].str),stack[top-1]);
		}
	}
...
#line 305 "parser.y" /* yacc.c:1646  */
    {
		...
		{
			int currscope=stack[top-1];
			int alcance=retornaralcance((yyvsp[-1].str),currscope);
			int tipo=retornartipo((yyvsp[-1].str),alcance);
...
#line 318 "parser.y" /* yacc.c:1646  */
    {
		...
		{
			int currscope=stack[top-1];
			int previous_scope=retornaralcance((yyvsp[-1].str),currscope);
			if(currscope==previous_scope)
				{printf("\nError : Redeclaracion of %s : Linea %d\n",(yyvsp[-1].str),printline());errc++;}
			else
			{
				insertar_dup((yyvsp[-1].str),(yyvsp[-2].ival),currscope);
		...
		else
		{
			int alcance=stack[top-1];
			//printf("%d",tipo);
			insertar((yyvsp[-1].str),(yyvsp[-2].ival));
			insertaralcance((yyvsp[-1].str),alcance);
			//verificar_alcance_actua($2,$4,stack[top-1]);
		}
	}
...
#line 340 "parser.y" /* yacc.c:1646  */
    {
		       ...
		       {
			int currscope=stack[top-1];
			int previous_scope=retornaralcance((yyvsp[-4].str),currscope);
			if(currscope==previous_scope)
				{printf("\nError : Redeclaration of %s : Line %d\n",(yyvsp[-4].str),printline());errc++;}
			else
			{

				insertar_dup((yyvsp[-4].str),ARRAY,currscope);
                                insertar_por_alcance((yyvsp[-4].str),(yyvsp[-5].ival),currscope);	//to insertar tipo to the correct identifier in case of multiple entries of the identifier by using alcance
                                if (itype==258) {insertar_index((yyvsp[-4].str),(yyvsp[-2].str));}
			}
		      }
		      else
		      {
			int alcance=stack[top-1];
                        insertar((yyvsp[-4].str),ARRAY);
			insertar((yyvsp[-4].str),(yyvsp[-5].ival));
			insertaralcance((yyvsp[-4].str),alcance);
                        if (itype==258) {insertar_index((yyvsp[-4].str),(yyvsp[-2].str));}
		      }
...
#line 374 "parser.y" /* yacc.c:1646  */
    {
				insertar((yyvsp[-1].str),STRUCT_VAR);
				}
